...
public class MotionBlur {
	 ...
	 * does nothing.
	 */
	public static boolean useAlphaChannelAsMask = true;
	 ...
	 */
	public MotionBlur(int FBO_W, int FBO_H, boolean hasDepth, boolean useBlending,
			boolean use32bitFBO) {
	 ...
	 *            fbo use higher precision than 16bits.
	 */
	public MotionBlur(int FBO_W, int FBO_H, FrameBuffer sceneIsCapturedHere,
	...
	private void gaussianBlur() {
		...
		blurPasses = (int)((blurDirection.x + blurDirection.y) / 2);
		original.bind(0);
		...
		for (int i = 0; i < blurPasses; i++) {
			...
			pingPongBuffer2.begin();
			{
				blurShader.begin();
				{
					blurShader.setUniformf("dir", -blurDirection.x, blurDirection.y);
	...
	public void setSize(int FBO_W, int FBO_H) {
		w = FBO_W;
		h = FBO_H;
		blurShader.begin();
		blurShader.setUniformf("size", FBO_W, FBO_W);
		blurShader.end();
	}
	...
	private Mesh createFullScreenQuad() {
		...
		Mesh tmpMesh = new Mesh(true, 4, 0, new VertexAttribute(Usage.Position, 2, ShaderProgram.POSITION_ATTRIBUTE), new VertexAttribute(Usage.TextureCoordinates, 2, ShaderProgram.TEXCOORD_ATTRIBUTE + "0"));
